...
Run reactive scopes twice when a source updates.
Throw an error if yields occur where they are not allowed.
Checks for indexes() and values() outputting primitive values.
Checks for values() input having duplicate values.
Checks for duplicate nested properties at same depth.
Checks for destruction of an active scope.
...
It is recommended to develop UI with strict mode and to disable it when pushing to production. In Roblox, production code compiles at O2 by default, so you do not need to worry about disabling strict mode unless you have manually enabled it.
