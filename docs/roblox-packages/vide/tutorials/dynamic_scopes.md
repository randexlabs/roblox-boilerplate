Dynamic Scopes
Eventually you may need a way to dynamically create and destroy UI elements resulting from source updates. Vide provides functions to help you do this, known as dynamic scope functions.
...
If toggled goes from truthy to falsey, beginning the timer, but then back to truthy before the timer finishes, the timer is cancelled and the scope is not destroyed.
