...
`const` makes the _binding_ immutable, but it does not make the _value_ immutable. For instance, a `const` variable referring to a table will still allow the table's contents to be mutated:
