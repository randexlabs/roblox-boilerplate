...
Returns the type of the object; for userdata objects that have a metatable with the `__type` field _and_ are defined by the host (not `newproxy`), returns the value for that key.
